...
# Purpose: Selectively backup Application Data folders using JSON profile
# ==============================================================================
...
Write-Host "`n=== APPDATA BACKUP ===" -ForegroundColor Cyan
...
# Load settings to get AppData profile
$settings = Load-JsonFile -FilePath (Join-Path $RootDir "settings.json")
$appDataProfile = if ($settings -and $settings.AppDataBackupProfile) { $settings.AppDataBackupProfile } else { @{ SelectedApps = @(); FolderMappings = @{}; ExcludePatterns = @() } }
$selectedApps = $appDataProfile.SelectedApps
$folderMap = if ($appDataProfile.FolderMappings) { $appDataProfile.FolderMappings } else { @{} }

if ($null -eq $selectedApps -or $selectedApps.Count -eq 0) {
    Write-Host "`n⚠ No apps selected in AppData profile." -ForegroundColor Yellow
    Write-Host "Run Option 1 + 2 (Get Inventory → Select Apps) first, then run Backup-AppData again." -ForegroundColor Yellow
    Write-Host "`nAlternatively, would you like to configure apps now? (Y/N): " -ForegroundColor Cyan -NoNewline
    $response = Read-Host
    
    if ($response -match "^(Y|Yes)$") {
        Write-Host "Opening app selection..." -ForegroundColor Green
...
# Load CSV for reference (to skip Store and WSL apps)
$csvPath = Join-Path $invDir $config.InventoryOutputCSV
$inventory = @()
if (Test-Path $csvPath) {
    try {
        $inventory = Import-Csv -Path $csvPath -ErrorAction SilentlyContinue
    } catch {
        Write-Host "⚠ Could not load inventory CSV for reference" -ForegroundColor Yellow
    }
}

# Build lookup for app environment and source
$appLookup = @{}
foreach ($row in $inventory) {
    $appLookup[$row.'Application Name'] = @{
...
Write-Host "`nProcessing apps from AppData profile..." -ForegroundColor Yellow
Write-Host "Selected apps: $($selectedApps.Count)" -ForegroundColor Cyan

foreach ($appName in $selectedApps) {
    # Look up app info from CSV
    $appInfo = $appLookup[$appName]
    
    # Skip Windows Store and WSL apps (only backup Windows user apps)
    if ($appInfo -and ($appInfo.Environment -match "WSL" -or $appInfo.Source -eq "Microsoft Store")) {
        Write-Host "`n⊘ Skipping: $appName (Source: $($appInfo.Source))" -ForegroundColor DarkGray
    ...
    if ($folderMap.ContainsKey($appName)) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($folderName)) {
            ...
            Save-JsonFile -Data $folderMap -FilePath $folderMapPath | Out-Null
        }
    }
